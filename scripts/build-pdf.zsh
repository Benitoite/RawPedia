...
echo "Verifying Keyboard Shortcuts source..."

if [[ ! -f "$CONTENTS_DIR/keyboard_shortcuts/index.md" ]]; then
  echo "❌ Missing source: $CONTENTS_DIR/keyboard_shortcuts/index.md"
  exit 1
fi

if ! grep -q '## Cross Platform Shortcuts' "$CONTENTS_DIR/keyboard_shortcuts/index.md"; then
  echo "❌ Keyboard Shortcuts source exists, but does not look like the real article."
  echo "❌ Expected to find: ## Cross Platform Shortcuts"
  exit 1
fi

echo "Keyboard Shortcuts source OK"
...
echo "Verifying generated Keyboard Shortcuts HTML..."

KS_PUBLIC="$SOURCE_DIR/keyboard_shortcuts/index.html"

if [[ ! -f "$KS_PUBLIC" ]]; then
  echo "❌ Missing generated page: $KS_PUBLIC"
  exit 1
fi

if ! grep -q 'Cross Platform Shortcuts' "$KS_PUBLIC"; then
  echo "❌ Generated Hugo page exists, but does not contain Keyboard Shortcuts body."
  echo "❌ Source markdown is OK, but Public/ is stale or was generated incorrectly."
  echo "❌ Regenerate RawPedia Public, then rerun this PDF script."
  exit 1
fi

echo "Generated Keyboard Shortcuts HTML OK"
...
def choose_qr_content_rel(title: str, rel: str) -> str:
    ...
    hard_source_aliases = {
        "keyboardshortcuts": "keyboard_shortcuts/index.md",
        "metadatacopymode": "Metadata_Copy_Mode/index.md",
        "preprocessing": "Preprocessing/index.md",
    }

    for key in {route_key, title_key}:
        wanted = hard_source_aliases.get(key)
        if wanted and wanted in qr_git_content_rels():
            return wanted
    ...
    candidates = sorted(set(candidates))
    candidates.sort(key=lambda item: (-qr_simple_score(item, title, rel), item))
    ...
    if qr_rel_is_redirect(chosen):
        ...
        if repaired:
            repaired.sort(key=lambda item: (-qr_simple_score(item, target, rel), item))
...
def strip_translations_footer(content: str) -> str:
    if not content:
        return content
    ...
    search_start = max(0, len(content) - 12000)
    tail = content[search_start:]

    # Matches:
    # <h2>Translations</h2>
    # <h2>Translations:</h2>
    # <h2><span id="Translations">Translations</span></h2>
    for m in re.finditer(r"<h[1-6]\b[^>]*>.*?</h[1-6]>", tail, flags=re.I | re.S):
        text = visible_text(m.group(0))
        text = re.sub(r"\s*\[\s*edit\s*\]\s*$", "", text, flags=re.I)
        text = text.strip(" :\u00a0")

        if text.lower() == "translations":
            return content[:search_start + m.start()].rstrip()

    # Matches paragraph/div/table/list footer variants.
    for m in re.finditer(
        r"<(p|div|section|table|ul|ol)\b[^>]*>.*?</\1>",
        tail,
        flags=re.I | re.S,
    ):
        text = visible_text(m.group(0)).strip()

        if re.match(r"(?i)^translations\s*:", text):
            return content[:search_start + m.start()].rstrip()
...
def page_kind(title: str, rel: str) -> tuple[int, str]:
    ...
    sections = [
        ("Part I — Getting Started", [
            "main page", "getting started", "download", "nightly", "install",
            "windows", "linux", "macos", "wayland", "file paths", "portable",
            "supported cameras", "features",
        ]),
        ("Part II — Interface and Workflow", [
            "editor", "file browser", "queue", "batch", "favorites", "exif",
            "iptc", "metadata", "keyboard shortcuts", "shortcut", "preferences",
            "preview modes", "saving", "external editor", "sidecar",
            "processing profile", "dynamic processing", "profile chooser",
            "metadata copy",
        ]),
        ("Part III — Raw Development Basics", [
            "demosaic", "demosaicing", "raw black", "raw white", "preprocessing",
            "dark frame", "dark-frame", "flat field", "flat-field",
            "white balance", "exposure", "black points", "white points",
            "clipping", "floating point", "toolchain pipeline", "bit depth",
            "8-bit", "16-bit",
        ]),
        ("Part IV — Tone, Contrast, and Exposure Tools", [
            "tone", "exposure", "shadows", "highlights", "dynamic range",
            "contrast", "tone mapping", "retinex", "haze", "graduated filter",
            "soft light", "vibrance", "rgb curves", "lab adjustments",
            "black-and-white", "black and white", "channel mixer", "edges",
            "microcontrast",
        ]),
        ("Part V — Color Management and Color Tools", [
            "color management", "color appearance", "ciecam", "cam16", "jzcz",
            "jzazbz", "hsv", "rgb and lab", "gamut", "icc", "dcp",
            "input profile", "film simulation", "film emulation", "film negative",
            "color toning", "chromaticity", "profile creator",
        ]),
        ("Part VI — Detail, Noise, and Sharpening", [
            "sharpen", "capture sharpening", "noise", "denoise", "impulse",
            "defringe", "chromatic aberration", "chromatic-aberration",
            "chromatic_aberration", "deconvolution", "detail", "wavelet levels",
            "wavelets",
        ]),
        ("Part VII — Geometry, Lens, and Output", [
            "crop", "resize", "lens", "geometry", "perspective", "vignetting",
            "watermark", "framing", "image file formats", "compression",
            "output", "gimp plugin",
        ]),
        ("Part VIII — Local Adjustments and Selective Editing", [
            "local adjustments", "local contrast", "local controls", "local lab",
            "selective editing", "spot removal", "mask", "red eye",
        ]),
        ("Part IX — Advanced Topics, Reference, and Contributing", [
            ...
            "general photography", "interact", "localization", "game changer",
            "game-changing",
        ]),
    ]
...
def add_pdf_named_destinations(s: str) -> str:
    """
    Do not add <a name="..."> duplicates for existing ids.
    WeasyPrint resolves internal PDF links from normal HTML id targets.
    Keeping this as a no-op preserves the call sites without creating
    duplicate id/name targets.
    """
    return s
...
def rewrite_article_links_to_manual(content: str, page_path: Path, manual_link_map: dict[str, str]) -> str:
    def anchor_repl(m):
        full = m.group(0)
        before_href = m.group(1)
        quote = m.group(2)
        href = m.group(3)
        after_href = m.group(4)

        if re.search(r'\bdata-keep-external-github=(["\'])1\1', full, flags=re.I | re.S):
            return full

        href_unescaped = html.unescape(href or "").strip()

        if href_unescaped.startswith("https://github.com/RawTherapee/RawPedia/blob/master/content/"):
            return full

        new_href = rewrite_one_manual_href(href_unescaped, page_path, manual_link_map)

        return (
            f'<a{before_href}'
            f'href={quote}{html.escape(new_href, quote=True)}{quote}'
            f'{after_href}>'
        )
...
def build_article_toc(content: str, page_id: str) -> str:
    ...
    # TOC from useful articles such as Keyboard Shortcuts and Download.
    if not headings:
        return ""
...
for page in all_pages_raw:
    ...
    if content_file and source_file_is_redirect(content_file):
        target = extract_redirect_target_from_source_text(read_text(content_file))
        real_file = find_content_file_for_redirect_target(target, content_file)

        if real_file:
            real_rel = str(real_file.relative_to(CONTENTS_DIR)).replace("\\", "/")
            real_route = re.sub(r"\.(md|markdown|html)$", "/index.html", real_rel, flags=re.I)

            replacement_page = SOURCE_DIR / real_route
            if replacement_page.exists():
            ...
            if content_file and source_file_is_redirect(content_file):
                ...
                if real_file:
                    real_rel = str(real_file.relative_to(CONTENTS_DIR)).replace("\\", "/")
                    rel = re.sub(r"\.(md|markdown|html)$", "/index.html", real_rel, flags=re.I)
                    title = title_from_doc(text, rel)
        
            # Skip empty translation hub pages when a real same-title article exists elsewhere.

    protected_article_ids = {
        "chromatic-aberration",
    ...
    page_id = make_id(rel)
    protected_article = page_id in protected_article_ids

    suppress, reason = should_suppress_page(title, rel, text)

    if suppress and not protected_article:
        if reason == "redirect":
    ...
    if not is_probably_english_page(page, title, text) and not protected_article:
        continue

    content = extract_main_or_body(text)

    if page_id == "keyboard-shortcuts":
        print("KS after extract_main_or_body chars:", len(content))
        print("KS has Cross Platform after extract:", "Cross Platform Shortcuts" in content)

    content = strip_bad_parts(content)
    content = strip_translations_footer(content)
    
    if page_id == "keyboard-shortcuts":
        print("KS after strip_bad_parts chars:", len(content))
        print("KS has Cross Platform after strip:", "Cross Platform Shortcuts" in content)
    ...
    def strip_duplicate_article_heading(content: str, title: str) -> str:
        ...
        if m and same_or_starts_with_title(m.group(0)):
            block_plain = clean_text(m.group(0))

            # Only remove a tiny duplicated title wrapper.
            # Do not remove a large article container whose text merely starts
            # with the article title.
            if len(block_plain) <= len(title_plain) + 120:
    ...
    content = add_pdf_named_destinations(content)
    ...
    is_major_article = article_is_major(content, title)
    ...
    if not has_meaningful_content(content, title) and not protected_article:
        continue
    ...
    if page_id == "keyboard-shortcuts":
        print("KS before infos.append chars:", len(content))
        print("KS has Cross Platform before append:", "Cross Platform Shortcuts" in content)
    infos.append({
        "path": page,
        "rel": rel,
        "id": page_id,
        ...
        "is_major_article": is_major_article,
        "github_url": github_url,
        "github_qr_uri": github_qr_uri,
        "aliases": aliases,
        "content": content,
    })
required_extra_routes = {
    "Keyboard_Shortcuts/index.html",
}

present_rels = {info["rel"] for info in infos}

for page in all_pages_raw:
    rel = rel_from_source(page).replace("\\", "/")

    if rel not in required_extra_routes:
        continue
...
# Guard against silently dropping per-article TOCs again.  Any selected
# article with at least one usable h2-h4 destination must have a local TOC.
...
if missing_article_tocs:
    print()
    print("❌ Per-article TOCs are missing for articles with section headings:")
    for title, rel in missing_article_tocs:
        print(f"   {title} [{rel}]")
    sys.exit(1)
...
present_ids = {info["id"] for info in infos}
missing_ids = sorted(required_route_needles - present_ids)

if missing_ids:
    print()
    print("❌ Required linked English articles are missing from manual:")
...
.manual-article.article-break-after {{
  break-after: page;
}}
...
@page articlepage:left {{
  ...
  @top-left {{ content: element(bookHeader); }}
...
@page articlepage:right {{
  ...
  @top-right {{ content: element(bookHeader); text-align: right; }}
...
/* RAWPEDIA_ARTICLE_TOP_PULLUP_CSS */
.article.article-major {{
  break-before: page;
...
.article.article-major > :first-child,
.article.article-major > .running-article-footer + .section-label,
.article.article-major > .section-label:first-of-type {{
...
.article.article-compact + .article.article-compact {{
  margin-top: 0;
}}

.article.article-compact + .article.article-major,
.article.article-major + .article.article-compact,
.article.article-major + .article.article-major {{
  margin-top: 0;
}}
...
.article-body figure img:not(.rt-small-icon-image),
.article-body .dereferenced-mediawiki-image img:not(.rt-small-icon-image),
.dereferenced-mediawiki-image img:not(.rt-small-icon-image),
.article-body img.rt-large-article-image {{
  display: block;
  width: 100%;
  max-width: 100%;
  height: auto;
...
.article-body img.rt-small-icon-image {{
...
/* Do not blindly blow up every standalone image; many RawPedia icon images
   are emitted as a single image inside a paragraph/div. */
.article-body > p > img:only-child:not([src*="icon"]):not([src*="Icon"]):not([src*="preview_mode"]):not([src*="Preview"]):not([src*="button"]):not([src*="Button"]):not([src*="toolbar"]):not([src*="Toolbar"]),
.article-body > div > img:only-child:not([src*="icon"]):not([src*="Icon"]):not([src*="preview_mode"]):not([src*="Preview"]):not([src*="button"]):not([src*="Button"]):not([src*="toolbar"]):not([src*="Toolbar"]) {{
  display: block;
  max-width: 100%;
  width: auto;
  height: auto;
...
.article-body a img,
.article-body img[src*="icon"],
.article-body img[src*="Icon"],
.article-body img[src*="preview_mode"],
.article-body img[src*="Preview"],
.article-body img[src*="button"],
.article-body img[src*="Button"],
.article-body img[src*="toolbar"],
.article-body img[src*="Toolbar"] {{
  width: auto;
  height: auto;
  max-width: 100%;
...
/* Prevent inline code from running out of article columns. Keep this in the
   initial stylesheet so the baseline and publisher-final renders paginate
   the article body identically. */
...
<h2>Reference Versions at Buildtime</h2>
...
""")
    ...
    def should_break_after_article(article_html: str) -> bool:
        plain = re.sub(r"<[^>]+>", " ", article_html)
        plain = re.sub(r"\s+", " ", plain).strip()

        SHORT_ARTICLE_WORD_LIMIT = 350

        return len(plain.split()) > SHORT_ARTICLE_WORD_LIMIT
...
""")
        ...
        if info.get("is_major_article", False):
            article_classes.append("article-major")
        else:
            article_classes.append("article-compact")
...
echo
echo "Classifying small/icon images..."

python3 - "$OUTPUT_HTML" <<'CLASSIFY_SMALL_IMAGES'
import re
import sys
import html
import struct
...
icon_name_re = re.compile(
    r"(?:icon|button|toolbar|preview[_-]?mode|indicator|logo|symbol)",
    flags=re.I,
)

def raster_size(path: Path):
    try:
...
def svg_size(path: Path):
    ...
    m = re.search(r"<svg\b[^>]*\bwidth=[\"']([^\"']+)[\"'][^>]*\bheight=[\"']([^\"']+)[\"']", txt, flags=re.I | re.S)
    if m:
        w = num(m.group(1))
        h = num(m.group(2))
        if w and h:
    ...
    m = re.search(r"<svg\b[^>]*\bviewBox=[\"']([^\"']+)[\"']", txt, flags=re.I | re.S)
    if m:
...
def add_class(tag: str, cls: str) -> str:
    m = re.search(r'\bclass=(["\'])(.*?)\1', tag, flags=re.I | re.S)
    ...
    if m:
        ...
        if cls not in classes:
            classes.append(cls)
        return tag[:m.start(2)] + " ".join(classes) + tag[m.end(2):]
...
def repl(m):
    global small_count, large_count, unknown_count

    tag = m.group(0)
    src_m = re.search(r'\bsrc=(["\'])(.*?)\1', tag, flags=re.I | re.S)
    ...
    path = Path(urllib.parse.unquote(urllib.parse.urlsplit(src).path))
    size = img_size(path)

    name_is_icon = bool(icon_name_re.search(path.name))
    ...
    if size:
        ...
        if max_dim <= 72 or (name_is_icon and max_dim <= 128 and min_dim <= 128):
            small_count += 1
            return add_class(tag, "rt-small-icon-image")

        # Large real image/figure: allowed to scale.
        large_count += 1
        return add_class(tag, "rt-large-article-image")

    # Unknown size: only trust filename clues.
    if name_is_icon:
        small_count += 1
        return add_class(tag, "rt-small-icon-image")
...
s2 = re.sub(r"<img\b[^>]*>", repl, s, flags=re.I | re.S)
html_path.write_text(s2, encoding="utf-8")

print(f"small/icon images marked: {small_count}")
print(f"large article images marked: {large_count}")
print(f"unknown image sizes: {unknown_count}")
CLASSIFY_SMALL_IMAGES
...
echo "🔗 Rescuing RawPedia article links back to internal manual links..."
...
source_redirect_aliases = {
    "keyboard-shortcuts": "Keyboard Shortcuts",
    "keyboard_shortcuts": "Keyboard Shortcuts",
    "metadata-copy-mode": "Metadata Copy Mode",
    "metadata_copy_mode": "Metadata Copy Mode",
    "preprocessing": "Preprocessing",
}

for redirect_slug, real_title in source_redirect_aliases.items():
    real_key = keyify(real_title)

    if real_key in manual:
        manual[keyify(redirect_slug)] = manual[real_key]
        
# Explicit legacy RawPedia aliases that do not match generated title/stem exactly.
manual_aliases = {
    "image-processing-tab": "#page-editor",
    "image_processing_tab": "#page-editor",
    "chromatic-aberration": "#page-chromatic-aberration",
    "command-line-options": "#page-command-line-options",
    "comparison-of-the-3-rawtherapee-noise-reduction-tools": "#page-comparison-of-the-3-rawtherapee-noise-reduction-tools",
    "dynamic-processing-profiles": "#page-dynamic-processing-profiles",
    "keyboard-shortcuts": "#page-keyboard-shortcuts",
    "keyboard_shortcuts": "#page-keyboard-shortcuts",
    "metadata-copy-mode": "#page-metadata-copy-mode",
    "preprocessing": "#page-preprocessing",
    "comparison-of-the-3-rawtherapee-noise-reduction-tools": "#page-about-noise-reduction",
    "the-file-browser-tab": "#page-file-browser",
    "the_file_browser_tab": "#page-file-browser",
    "keyboard-shortcuts": "#page-keyboard-shortcuts",
    "keyboard_shortcuts": "#page-keyboard-shortcuts",
    "lens-geometry": "#page-lens-geometry",
    "lens/geometry": "#page-lens-geometry",
    "lens--geometry": "#page-lens-geometry",
    "shadows-highlights": "#page-shadows-highlights",
    "shadows/highlights": "#page-shadows-highlights",
    "shadows--highlights": "#page-shadows-highlights",
    "wavelet": "#page-wavelet-levels",
    "wavelets": "#page-wavelet-levels",
}

for k, v in manual_aliases.items():
    if v.startswith("#") and re.search(
        rf'\bid=["\']{re.escape(v[1:])}["\']',
        s,
        flags=re.I,
    ):
        manual[keyify(k)] = v
...
def repl(m):
    ...
    # Keep real downloadable assets external.
    if ext in {".png", ".jpg", ".jpeg", ".gif", ".svg", ".webp", ".pdf", ".zip", ".7z", ".dmg", ".exe", ".pp3", ".icc", ".icm", ".dcp"}:
        return m.group(0)

    path = urllib.parse.unquote(parsed.path)

    # Homepage is allowed to stay external.
    if path in {"", "/"}:
        return m.group(0)

    # Localized article links should resolve to the English manual article.
    # Handles both:
    #   /fr/color_management_addon/
    #   /Color_Management_addon/fr#Balance_des_blancs
    localized_path_variants = {path}

    localized_path_variants.add(re.sub(r"^/(fr|es|it|jp|ja|pt|de|ca|ct|zh|cn|ru|nl|pl|tr)/", "/", path, flags=re.I))
    localized_path_variants.add(re.sub(r"/(fr|es|it|jp|ja|pt|de|ca|ct|zh|cn|ru|nl|pl|tr)/?$", "", path, flags=re.I))

    keys_to_try = [keyify(href)]

    for path_variant in localized_path_variants:
        keys_to_try.append(keyify(path_variant))
    ...
    # These are intentionally suppressed/non-article pages.
    if any(keyify(path_variant) in {"irc", "linux-gtk2"} for path_variant in localized_path_variants):
        return m.group(0)

    unresolved.add(href)
    return m.group(0)

s2 = re.sub(r'\bhref=(["\'])(https?://[^"\']+)\1', repl, s, flags=re.I | re.S)
html_path.write_text(s2, encoding="utf-8")

print(f"RawPedia article links rescued to internal links: {changed}")
print(f"Unresolved RawPedia article links: {len(unresolved)}")

if unresolved:
    print()
    print("❌ RawPedia article links still unresolved:")
    for href in sorted(unresolved)[:160]:
        print(f"   {href}")
    sys.exit(1)
...
# Remove the old publisher-final style too. Otherwise reruns keep stale CSS
# while replacing only the injected HTML pages.
import re
s = re.sub(
    r'\s*<style\b[^>]*\bid=["\']publisher-final-pages-style["\'][^>]*>.*?</style>\s*',
    "\n",
    s,
    flags=re.I | re.S,
)
...
for i in range(pad_needed):
    sections.append(f"""
<section class="publisher-blank-page" aria-label="Publisher padding page {i + 1}">
  <div class="publisher-page-inner">
...
sections.append(f"""
<section class="publisher-final-blank-page" aria-label="Publisher final blank page">
  <div class="publisher-page-inner">
    <img src="{icon_uri}" alt="">
  </div>
</section>

<section class="publisher-final-icon-page" aria-label="Publisher final icon page">
  <div class="publisher-page-inner publisher-page-inner-large-icon">
...
.publisher-page-inner-large-icon {
  padding-top: 3.2in;
}

.publisher-page-inner-large-icon img {
  width: 1.65in;
  height: 1.65in;
  max-width: 1.65in;
  max-height: 1.65in;
...
if True:
    ...
    if "margin-top: -0.19in" not in s:
        print("❌ Article top pull-up CSS was not present in final HTML.")
        sys.exit(1)
...
if [[ "$FINAL_PAGE_TOTAL" != "$EXPECTED_FINAL_TOTAL" ]]; then
  ...
  echo "❌ Refusing to repair the final PDF with qpdf because that can break internal PDF link destinations."
  echo "   Fix the injected HTML/page-break logic so WeasyPrint renders the expected page count in one pass."
  exit 1
fi
...
""")
        ...
        if info.get("has_article_toc", False):
            article_classes.append("article-major")
        else:
            article_classes.append("article-compact")
...
if True:
    ...
    # ---- Insert CSS for article-body code wrapping ----
    # Find the <style> block for article-body and insert the CSS after p/li/td/table/img rules.
    # The CSS below is inserted after the existing article-body paragraph/list/table/image styling.
    extra_code_css = """
/* Prevent inline code from running out of article columns. */
.article-body :not(pre) > code,
.article-body p code,
.article-body li code,
.article-body td code {
...
"""
    ...
    # Try to insert after the article-body's p/ul/ol/td/table/img rules in the <style> section.
    import re
    def insert_code_css(s):
        # Find the <style> block containing .article-body rules.
        m = re.search(
            r"(<style[^>]*>.*?)(\.article-body[^{]*\{[^}]*\}.*?)(</style>)",
            s,
            flags=re.I | re.S,
        )
        if m:
            # Insert after the article-body paragraph/list/table/image rules.
            before = s[:m.end(2)]
            after = s[m.end(2):]
            return before + extra_code_css + after
        else:
            # If not found, append to <head>
            return s.replace("</head>", "<style>" + extra_code_css + "</style>\n</head>")
